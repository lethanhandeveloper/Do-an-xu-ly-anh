...
def laplace_sharpen(img, isLaplaceSharpen):
    ...
    return img
def prewitf_filter(img, isPrewitf):
    Hkernel = np.array([[1, 0, -1],
                        [1, 0, -1],
                        [1, 0, -1]])
    Vkernel = np.array([[1, 1, 1],
                        [0, 0, 0],
                        [-1, -1, -1]])

    if(isPrewitf == True):
        return cv2.filter2D(img, ddepth=-1, kernel=Vkernel) + cv2.filter2D(img, ddepth=-1, kernel=Hkernel)

    return img

def prewitf_sharpen(img, isChar):
    if(isChar == True):
        return img - prewitf_filter(img, True)
    return img
def char_filter(img, isChar):
    Hkernel = np.array([[3, 0, -3],
                        [10, 0, -10],
                        [3, 0, -3]])
    Vkernel = np.array([[3, 10, 3],
                        [0, 0, 0],
                        [-3, -10, -3]])
...
def char_sharpen(img, isChar):
    if(isChar == True):
        return img - char_filter(img, True)
    return img