...
from crop_form import Crop_Form
...
class MainWindow():
    ...
    def __init__(self):
        ...
        self.uic.actionPrewitt.triggered.connect(self.prewitfSharpen)
        self.uic.actionScharr.triggered.connect(self.charSharpen)

        self.uic.actionGamma.triggered.connect(self.changeGammaValue)

        #equalize_his_button
        self.uic.equalize_his_button.clicked.connect(self.equalize_his)
        self.uic.clear_his_button.clicked.connect(self.clear_his)

        #roberts edge detection
        self.uic.actionRoberts.triggered.connect(self.robertsEdgeDetection)
        self.uic.actionCrop.triggered.connect(self.openCropForm)
        
       
        #segmentation
        self.uic.kmean_button.clicked.connect(self.toggleKmeanSegment)
        if self.new_image==None:
            
            self.image = cv2.imread("./images/default.jpg")
    ...
    def toggleKmeanSegment(self):
        if(self.isKmean == False):
            self.isKmean = True
        elif():
            self.isKmean = False
        self.threadsegmen=Segmentation(self.new_image, self.uic.cluster_kmean_spin.value(), self.uic.iteration_kmean_spin.value(), self.isKmean)
        self.threadsegmen.ImageUpdate.connect(self.setImg)
        self.threadsegmen.Dialog.connect(self.dialog)
        self.threadsegmen.start()
        
    def dialog(self):
        self.dlg = QMessageBox(self.main_win)
        self.dlg.setWindowTitle("Segmentation")
        self.dlg.setText("Segmentation")
        self.dlg.exec()
    def setImg(self,imgse):
        self.new_image=imgse
        self.showImage(True)
    def openCropForm(self):
        self.crop_window = QtWidgets.QMainWindow()

        self.uicrop = Crop_Form(self.new_image)
        self.uicrop.setupUi(self.crop_window)

        self.crop_window.show()
    def equalize_his(self):
    ...
    def sobelSharpen(self):
        ...
        self.showImage(False)
    def prewitfSharpen(self):
        if (self.isPrewitf == True):
            self.isPrewitf = False
        else:
            self.isPrewitf = True

        self.showImage(False)
    def charSharpen(self):
        if (self.isChar == True):
            self.isChar = False
        else:
            self.isChar = True
    ...
    def updateChange(self):
        ...
        self.new_image = filter_module.prewitf_filter(self.new_image, self.isPrewitf)
        self.new_image = filter_module.char_filter(self.new_image, self.isChar)
        self.new_image = filter_module.prewitf_sharpen(self.new_image, self.isPrewitf)
        self.new_image = filter_module.char_sharpen(self.new_image, self.isChar)
